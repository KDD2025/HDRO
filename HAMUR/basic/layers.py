...
class EmbeddingLayerHDRO(nn.Module):
    """General Embedding Layer.
    We save all the feature embeddings in embed_dict: `{feature_name : embedding table}`.
    ...
    def __init__(self, features):
        ...
        for fea in features:
            if fea.name in self.embed_dict:  #exist
                continue
            if isinstance(fea, SparseFeature) and fea.shared_with == None:
                embedding_layer = fea.get_embedding_layer()
                self.init_embedding_parameters(embedding_layer)
                self.embed_dict[fea.name] = embedding_layer
            elif isinstance(fea, SequenceFeature) and fea.shared_with == None:
                embedding_layer = fea.get_embedding_layer()
                self.init_embedding_parameters(embedding_layer)
                self.embed_dict[fea.name] = embedding_layer
    ...
    def init_embedding_parameters(self, embedding_layer):
...
class EmbeddingLayerMultiDomain(nn.Module):
    """General Embedding Layer.
    We save all the feature embeddings in embed_dict: `{feature_name : embedding table}`.
    ...
    Shape:
        - Input:
            x (dict): {feature_name: feature_value}, sequence feature value is a 3D tensor with shape:`(batch_size,domain_num, seq_len)`,\
                      sparse/dense feature value is a 2D tensor with shape `(batch_size,domain_num)`.
            features (list): the list of `Feature Class`. It is means the current features which we want to do embedding lookup.
            squeeze_dim (bool): whether to squeeze dim of output (default = `False`).
        - Output:
            - if input Dense: `(batch_size, domain_num, num_features_dense)`.
            - if input Sparse: `(batch_size, domain_num, num_features, embed_dim)` or  `(batch_size,domain_num, num_features * embed_dim)`.
            - if input Sequence: same with input sparse or `(batch_size,domain_num, num_features_seq, seq_length, embed_dim)` when `pooling=="concat"`.
            - if input Dense and Sparse/Sequence: `(batch_size, domain_num,num_features_sparse * embed_dim)`. Note we must squeeze_dim for concat dense value with sparse embedding.
    """
    ...
    def forward(self, x, features, squeeze_dim=False):
        ...
        for fea in features:
            if isinstance(fea, SparseFeature):
                if fea.shared_with == None:
                    b,d = x[fea.name].shape #batch size, domain size
                    emb = self.embed_dict[fea.name](x[fea.name].long().reshape(-1))
                    emb = emb.reshape(b,d,-1)
                    sparse_emb.append(emb.unsqueeze(2)) # batch_size, domain_num, 1, Embedding_size
                else:
                    b,d = x[fea.name].shape #batch size, domain size
                    emb = self.embed_dict[fea.shared_with](x[fea.name].long().reshape(-1))
                    emb = emb.reshape(b,d,-1)
                    sparse_emb.append(emb.unsqueeze(2)) # batch_size, domain_num, 1, Embedding_size

            # 未对此做修改，因为未涉及到 SequenceFeature
            elif isinstance(fea, SequenceFeature):
                if fea.pooling == "sum":
                    pooling_layer = SumPooling()
                elif fea.pooling == "mean":
                ...
                dense_values.append(x[fea.name].float().unsqueeze(2))  # .unsqueeze(1).unsqueeze(1)

        if len(dense_values) > 0:
            dense_exists = True
            dense_values = torch.cat(dense_values, dim=2)
        if len(sparse_emb) > 0:
            sparse_exists = True
        ...
        if squeeze_dim:  # Note: if the emb_dim of sparse features is different, we must squeeze_dim
            if dense_exists and not sparse_exists:  # only input dense features
                ...
                return sparse_emb.flatten(start_dim=2)  # squeeze dim to : [batch_size,domain_num ,num_features*embed_dim]
            elif dense_exists and sparse_exists:
                return torch.cat((sparse_emb.flatten(start_dim=2), dense_values),
                                 dim=2)  # concat dense value with sparse embedding
            else:
                raise ValueError("The input features can note be empty")
        else:
            if sparse_exists:
