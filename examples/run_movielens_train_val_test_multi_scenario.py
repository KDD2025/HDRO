...
from HAMUR.Utility.tools import validate,getSeqDict,saveModel,readSeqDict,seq2str
from HAMUR.models.HDRO import HDRO_learning_w
...
def get_movielens_data_rank_multidomain(data_path="/home/zhangyabin/Multi-scenarios-Robust/train_val_test_split/ml-1m"):
    ...
    df_test = pd.read_csv(data_path + f'/test_dataset_{args.dis_scenario}_{args.dis_interaction}.csv')
    # print("df_test",df_test[:3])
    # exit()
    del df_train["label"]
    del df_val["label"]
    del df_test["label"]
    ...
    n_items = len(set(data["movie_id"].tolist()))
    domain_num = 3

    col_names = data.columns.values.tolist()
    dense_cols = ['age']
    sparse_cols = ['user_id', 'movie_id', 'gender', 'occupation', 'zip', "cate_id", "domain_indicator"]
...
def main(model_name_type,dataset_path, model_name, epoch, learning_rate, batch_size, weight_decay, device, save_dir, seed):
    ...
    dense_feas, sparse_feas, x_train, y_train, x_val, y_val, x_test, y_test,domain_num,n_items = get_movielens_data_rank_multidomain(dataset_path)
    ...
    if model_name_type == "Base":
        if model_name == "mlp":
            ...
            model = Mlp_2_Layer_SharedBottom(dense_feas + sparse_feas, domain_num=domain_num, fcn_dims=[256, 128])
        elif model_name == "MMoE":
            model = Mlp_2_Layer_MMoE(dense_feas + sparse_feas, domain_num=domain_num, fcn_dims=[256, 128])
        elif model_name == "PLE":
            model = Mlp_2_Layer_PLE(dense_feas + sparse_feas, domain_num=domain_num, fcn_dims=[256, 128])
        elif model_name == "AITM":
            model = Mlp_2_Layer_AITM(dense_feas + sparse_feas, domain_num=domain_num, fcn_dims=[256, 128])
        elif model_name == "STAR":
            model = Mlp_2_Layer_STAR(dense_feas + sparse_feas, domain_num=domain_num, fcn_dims=[256, 128])
    ...
    # add Weight V IN inter-level
        ...
        seq_num, seq_dict = getSeqDict(train_dataloader)
        model = HDRO_learning_w(baseModel=model_name,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset_path', default="/home/zhangyabin/Multi-scenarios-Robust/train_val_test_split/ml-1m") # /home/zhangyabin/Multi-scenarios-Robust/train_val_test_split/kuairand4/
    ...
    parser.add_argument('--balance_v_loss', '-alpha', type=float, default=0.01, help='balance of interaction level KL loss')
    ...
    parser.add_argument('--dis_interaction', '-tau2', type=float, default=0.8,
                        help='balance_data_distribution_scenario in multi domains')
    parser.add_argument('--remove_w', type=float, default=0,
                        help='ablation study')
