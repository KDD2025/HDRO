...
from HAMUR.Utility.tools import validate,getSeqDict,saveModel,readSeqDict,seq2str,getSeqDict_kuairand
from HAMUR.models.HDRO import HDRO_learning_w,HDRO_learning_w_douban,HDRO_learning_w_kuairand
...
def get_douban_data_rank_multidomain(data_path="/home/zhangyabin/Multi-scenarios-Robust/train_val_test_split/douban"):
    ...
    # print("data",set(data["domain_indicator"].tolist()))
    # exit()
    n_items = len(set(data["item_id"].tolist()))
    # user_id = len(set(data["user_id"].tolist()))
    #
    domain_num = 3

    col_names = data.columns.values.tolist()
    dense_cols = ['domain_id']
...
def main(model_name_type,dataset_path, model_name, epoch, learning_rate, batch_size, weight_decay, device, save_dir, seed):
    ...
    dense_feas, sparse_feas, x_train, y_train, x_val, y_val, x_test, y_test,domain_num,n_items = get_douban_data_rank_multidomain(dataset_path)
    ...
    if model_name_type == "Base":
        if model_name == "mlp":
            ...
            model = DCN_MD(features=dense_feas + sparse_feas, num_domains=domain_num, n_cross_layers=2,
                           mlp_params={"dims": [256, 128]})
        elif model_name == "dcn_md_adp":
            model = DCN_MD_adp(features=dense_feas + sparse_feas, num_domains=domain_num, n_cross_layers=2, k=30,
                               mlp_params={"dims": [256, 128]}, hyper_dims=[128])
        elif model_name == "wd_md":
            model = WideDeep_MD(wide_features=dense_feas, num_domains=domain_num, deep_features=sparse_feas,
                                mlp_params={"dims": [256, 128], "dropout": 0.2, "activation": "relu"})
        elif model_name == "wd_md_adp":
            model = WideDeep_MD_adp(wide_features=dense_feas, num_domains=domain_num, deep_features=sparse_feas, k=45,
                                    mlp_params={"dims": [256, 128], "dropout": 0.2, "activation": "relu"},
                                    hyper_dims=[128])
        elif model_name == "SharedBottom":
            model = Mlp_2_Layer_SharedBottom_kuairand(dense_feas + sparse_feas, domain_num=domain_num,
                                                      fcn_dims=[256, 128])
    ...
    # add Weight V IN inter-level
        ...
        seq_num, seq_dict = getSeqDict_kuairand(train_dataloader)
        model = HDRO_learning_w_douban(baseModel=model_name,
        ...
        import csv
        current_time = int(datetime.now().strftime("%Y%m%d%H%M%S"))
        csv_file_path = os.path.join(args.result_path, f"{args.model_name_type}_{args.data_name}_{model_name}_{seed}_{current_time}.csv")
        #with open(model_name+"_"+str(seed)+"_"+str(current_time)+'.csv', "w", newline='') as f:
        with open(csv_file_path, "w", newline='') as f:
            writer = csv.writer(f)
            writer.writerow(['model', 'seed', 'auc', 'log', 'auc1', 'log1', 'auc2', 'log2', 'auc3', 'log3'])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset_path', default="/home/zhangyabin/Multi-scenarios-Robust/train_val_test_split/douban") # /home/zhangyabin/Multi-scenarios-Robust/train_val_test_split/kuairand4/
    ...
    parser.add_argument('--learning_rate_v', type=float, default=1e-3)
    parser.add_argument('--learning_rate_vw', type=float, default=1e-3)
    parser.add_argument('--learning_rate_w', type=float, default=1e-3)
    parser.add_argument('--learning_rate_c', type=float, default=1e-3)
    parser.add_argument('--learning_rate_f', type=float, default=1e-3)
    parser.add_argument('--num_scenarios', type=int, default=3)
    parser.add_argument('--embedding_dim', type=int, default=16)
    parser.add_argument('--balance_v_loss', '-alpha', type=float, default=0.6, help='balance of interaction level KL loss')
    parser.add_argument('--balance_cluster_loss', '-beta', type=float, default=0.6, help='balance of interaction cluster loss')
    ...
    main(args.model_name_type,args.dataset_path, args.model_name, args.epoch, args.learning_rate, args.batch_size, args.weight_decay, args.device, args.save_dir, args.seed)
