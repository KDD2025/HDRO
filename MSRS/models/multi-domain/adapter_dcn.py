...
class DCN_MD_old(torch.nn.Module):
    """
    single-domain Deep & Cross Network
    """
...
class DCN_MD(torch.nn.Module):
    ...
    def __init__(self, features, num_domains, n_cross_layers, mlp_params):
        ...
        for d in range(num_domains):
            ...
            domain_specific.append(self.cn)

            domain_specific.append(self.mlp)

            domain_specific.append(self.linear)
    ...
    def forward(self, x):
        ...
        for d in range(self.num_domains):
            ...
            domain_input_cn = model_list[0](domain_input)  # cn
            domain_input_mlp = model_list[1](domain_input)  # mlp
            x_stack = torch.cat([domain_input_cn, domain_input_mlp], dim=1)
            y = model_list[2](x_stack)  # linear
            out.append(torch.sigmoid(y))
